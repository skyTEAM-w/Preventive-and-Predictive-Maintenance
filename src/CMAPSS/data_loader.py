...
def set_seed(time_seed):
    torch.manual_seed(time_seed)
    torch.cuda.manual_seed(time_seed)
    np.random.seed(time_seed)
    pass


def load_dataset(read_path, sub_name, max_life, cluster, train, test, finaltest):
    print("Loading dataset ", sub_name)
    if train or test:
        if cluster:
            data_set = pd.read_csv(read_path + 'train_FD' + str(sub_name) + '_cluster.csv')
        else:
            data_set = pd.read_csv(read_path + 'train_FD' + str(sub_name) + '.csv')

    elif finaltest:
        if cluster:
            data_set = pd.read_csv(read_path + "test_FD" + str(sub_name) + '_cluster.csv')
        else:
            data_set = pd.read_csv(read_path + "test_FD" + str(sub_name) + '.csv')

    else:
        print('====Please Check the para=====')

    if train:
        data_set['RUL'] = compute_rul(read_path, sub_name, data_set, max_life, finaltest=False)
        print('\n\t Train FD ' + sub_name + 'input shape:', data_set.shape)

    elif test:
        value = []
        unique_id = pd.unique(data_set['id'])
        unique_20 = unique_id[-int(len(unique_id) * 0.2):]
        data_group = data_set.groupby('id', sort=False)

        for id, data in data_group:
            if id in unique_20:
                value.append(data)

        data_set = pd.concat(value, ignore_index=True, sort=False)

        data_set['RUL'] = compute_rul(read_path, sub_name, data_set, max_life, finaltest=False)
        print('\n\t Test FD ' + sub_name + 'input shape:', data_set.shape)

    else:
        data_set['RUL'] = compute_rul(read_path, sub_name, data_set, max_life, finaltest=True)
        print('\n\t FinalTest FD ' + sub_name + 'input shape:', data_set.shape)
...
def knee_rul(cycle_list, max_cycle, MAX_LIFE):
    knee_RUL = []
    if max_cycle > MAX_LIFE:
        knee_point = max_cycle - MAX_LIFE

        for i in range(0, len(cycle_list)):
            if i < knee_point:
                knee_RUL.append(MAX_LIFE)
            else:
                temp = knee_RUL[i - 1] - (MAX_LIFE / (max_cycle - knee_point))
                knee_RUL.append(temp)
    else:
        knee_point = MAX_LIFE
        print("=========== knee_point < MAX_LIFE ===========")
        for i in range(0, len(cycle_list)):
            knee_point -= 1
...
def compute_rul(read_path, sub_dataset, data_set, max_life, finaltest=False, id='id'):
    MAX_LIFE = max_life
    rul = []
    true_rul = pd.read_csv(read_path + 'RUL_FD' + sub_dataset + '.csv')
    unique_set = pd.unique(data_set['id'])
    for _id in unique_set:
        unique_id = data_set[data_set[id] == _id]
        cycle_list = unique_id['cycle'].tolist()
        # print(cycle_list)
        ...
        rul.extend(knee_rul(cycle_list, max_cycle, MAX_LIFE))

    print('Length of RUL ', len(rul))
    return rul
    pass
...
def get_scaleing(data, scaler, scaler_range, train):
    ...
    if "operating_condition" in data.columns:
        ...
        if train:
            scalingparams = {'scaler': scaler, 'scaler_range': scaler_range}
            ...
            for operating_condition, data in groupby_oc:
                ...
                if scalingparams['scaler'] == 'mm':
                    ...
                    scaled_data = (((data[sensors] - min_) / (max_ - min_)) * (
                            scalingparams['scaler_range'][1] - scalingparams['scaler_range'][0])) + \
...
def sequence_length_generator(scaled_data, window_size, cols_wo_na):
    if "operating_condition" in scaled_data.columns:
        input_features = ["s2", "s3", "s4", "s7", "s8", "s9", "s11", "s12", "s13", "s14", "s15", "s17", "s20",
                          "s21"]  # ,"oc_0", "oc_1", "oc_2", "oc_3", "oc_4", "oc_5"]
    else:
        input_features = ["s2", "s3", "s4", "s7", "s8", "s9", "s11", "s12", "s13", "s14", "s15", "s17", "s20", "s21"]

    groupby_scaled_data = scaled_data.groupby('id', sort=False)

    sequences = []
    rul = []

    for id, data in groupby_scaled_data:
    ...
    sequences = sequences.view(sequences.shape[0], 1, window_size,
                               len(input_features))  # shape = [RUl , channel, window_size, feature]
...
def plot_sensors(sensor_data, scaled=True):
    ...
    sensor_plot = sensor_data.groupby('id', sort=False)
    ...
    for id, data in sensor_plot:
        data = data[sensors]
        data.plot(subplots=True, figsize=(10, 10), layout=(7, 3), title=title + str(id))
...
class train_dataset(Dataset):
    ...
    def to_preprocess(self):
        training_set = load_dataset(self.read_path, self.sub_dataset, self.max_life, self.cluster, train=True,
                                    test=False, finaltest=False)
...
class test_dataset(Dataset):
    ...
    @staticmethod
    def to_preprocess(self):
        testing_set = load_dataset(self.read_path, self.sub_dataset, self.max_life, self.cluster, train=False,
                                   test=True, finaltest=False)
...
class finaltest_dataset(Dataset):
    ...
    @staticmethod
    def to_preprocess(self):
        final_testing_set = load_dataset(self.read_path, self.sub_dataset, self.max_life, self.cluster, train=False,
...
def LoadCMAPSS(read_path, sub_dataset, window_size, max_life, scaler, scaler_range, shuffle, batch_size, cluster=True):
    if shuffle:
        ...
        tr_dataset = train_dataset(read_path, sub_dataset, window_size, max_life, scaler, scaler_range, cluster=True)
        print('\n\t Length of none shuffled train data: ', len(tr_dataset))
    ...
    final_dataset = finaltest_dataset(read_path, sub_dataset, window_size, max_life, scaler, scaler_range,
                                      cluster=True)
    print('\n\t Length of finaltest data: ', len(final_dataset))
    finaltest_loader = DataLoader(final_dataset, batch_size, shuffle=False)

    return list(enumerate(train_loader)), list(
        enumerate(test_loader)), train_loader, test_loader, unshuffle_train_loader, finaltest_loader
