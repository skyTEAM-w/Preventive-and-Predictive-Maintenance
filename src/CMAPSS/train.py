...
def Train(train_loader, test_loader, unshuffle_train_loader, finaltest_loader, model, optimizer, loss_func,
          num_epochs=100):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (data_tr, label_tr) in enumerate(train_loader):
            ...
            writer.add_scalar('loss', loss_tr.item())
        ...
        if epoch == num_epochs - 1:
            if unshuffle_train_loader is not None:
                train_output = []
                for i, (data_uns_tr, label_un_tr) in enumerate(unshuffle_train_loader):
                    data_uns_tr = data_uns_tr.to(device)
                    label_un_tr = label_un_tr.to(device)
