...
import src.CMAPSS.data_loader as CMAPSS_Dataloader
...
def train_actual_predicted(read_path, sub_dataset, window_size, max_life, train_output, alpha_grid, alpha_low,
    ...
    train_data = CMAPSS_Dataloader.load_dataset(read_path, sub_dataset, max_life, train=True, test=False,
                                                finaltest=False, cluster=True)
    train_data_gp = train_data.groupby('id', sort=False)
    train_data_reduce = []

    for id, data in train_data_gp:
    ...
    train_rul_group = train_data.groupby('id', sort=False)
    max_plots = []

    for id, data in train_rul_group:
        ...
        plt.grid(which="both", axis="both", alpha=alpha_grid)
        ...
        ax.set_title('FD' + sub_dataset + " Train Engine Unit-" + str(id), fontsize=26)
...
def test_actual_predicted(read_path, sub_dataset, window_size, max_life, test_output, alpha_grid, alpha_low, alpha_high,
    ...
    test_data = CMAPSS_Dataloader.load_dataset(read_path, sub_dataset, max_life, train=False, test=True,
                                               finaltest=False, cluster=True)
    test_data_gp = test_data.groupby('id', sort=False)
    test_data_reduce = []

    for id, data in test_data_gp:
        data = data.iloc[window_size - 1:, :]
    ...
    test_rul_group = test_data.groupby('id', sort=False)
    max_plots = []

    for id, data in test_rul_group:
        actual_rul = data[['cycle', 'RUL']]
        actual_rul = actual_rul.set_index('cycle')
        predicted_rul = data[['cycle', 'Predicted RUL']]
        predicted_rul = predicted_rul.set_index('cycle')
        ...
        plt.plot(predicted_rul, color=_COLORS[0], alpha=alpha_high, linestyle="solid", linewidth=4,
                 label='Predicted RUL')
        plt.plot(actual_rul, color=_COLORS[1], alpha=alpha_high, linestyle="dashed", linewidth=4, label='Actual RUL')
        ax.set_title('FD' + sub_dataset + " Test Engine Unit-" + str(id), fontsize=26)
        ax.legend(loc="upper right", fontsize=16)
        max_plots.append(id)
...
def fianltest_actual_vs_predicted(read_path, sub_dataset, window_size, max_life, finaltest_output, alpha_grid,
    ...
    finaltest_data = CMAPSS_Dataloader.load_dataset(read_path, sub_dataset, max_life, train=False, test=False,
                                                    finaltest=True, cluster=True)
    finaltest_data_groupped = finaltest_data.groupby('id', sort=False)
    finaltest_data_reduced = []

    for id, data in finaltest_data_groupped:
        data = data.iloc[window_size - 1:, :]
    ...
    finaltest_rul_group = finaltest_data.groupby('id', sort=False)
    ...
    for id, data in finaltest_rul_group:
        if id in plot_ids:
            actual_rul = data[['cycle', 'RUL']]
            actual_rul = actual_rul.set_index('cycle')
            predicted_rul = data[['cycle', 'Predicted RUL']]
            ...
            plt.plot(predicted_rul, color=_COLORS[0], alpha=alpha_high, linestyle="solid", linewidth=4,
                     label='Predicted RUL')
            plt.plot(actual_rul, color=_COLORS[1], alpha=alpha_high, linestyle="dashed", linewidth=4,
                     label='Actual RUL')
            ax.set_title('FD' + sub_dataset + " Final Test Engine Unit-" + str(id), fontsize=26)
            ax.legend(loc="upper right", fontsize=16)
            fig.tight_layout()
            plt.show()
