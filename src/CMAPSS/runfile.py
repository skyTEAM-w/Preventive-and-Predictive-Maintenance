...
import src.CMAPSS.clustering as CMPASS_Clustering
import src.CMAPSS.data_loader as CMAPSS_Dataloader
import src.CMAPSS.train as CMAPSS_TrainLoop
import src.CMAPSS.visualize as CMAPSS_PlotFunctions
...
alpha_high = 0.9
_COLORS = ["green", "teal"]
read_path = r"../../data/output/CMAPSS/"
...
'''
train_data, test_data = CMPASS_Clustering.cluster(read_path, sub_dataset, save_model=True)
...
'''
_, _, train_loader, test_loader, unshuffle_train_loader, finaltest_loader = CMAPSS_Dataloader.LoadCMAPSS(read_path,
                                                                                                         sub_dataset,
                                                                                                         window_size,
                                                                                                         max_life,
                                                                                                         scaler,
...
'''
train_loss_epoch, test_loss_epoch, train_output, test_output, finaltest_output = CMAPSS_TrainLoop.Train(train_loader,
                                                                                                        test_loader,
...
'''
CMAPSS_PlotFunctions.train_actual_predicted(read_path, sub_dataset, window_size, max_life, train_output, alpha_grid,
...
'''
CMAPSS_PlotFunctions.test_actual_predicted(read_path, sub_dataset, window_size, max_life, test_output, alpha_grid,
                                           alpha_low, alpha_high, _COLORS)
...
'''
CMAPSS_PlotFunctions.loss_plot(sub_dataset, train_loss_epoch, test_loss_epoch, alpha_grid, alpha_low, alpha_high,
                               _COLORS)
...
'''
socre = CMAPSS_PlotFunctions.fianltest_actual_vs_predicted(read_path, sub_dataset, window_size, max_life,
                                                           finaltest_output, alpha_grid,
                                                           alpha_low, alpha_high, _COLORS)
model_save_path = 'results\\' + time.strftime('%Y%m%d%H%M_') + sub_dataset + '.pth'
