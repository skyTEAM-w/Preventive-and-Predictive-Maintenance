...
def cluster(read_path, sub_dataset, save_model=False, model_filename='kmeans_model.joblib'):
    ...
    ax.scatter(test_data['setting1'], test_data['setting2'], test_data['setting3'],
               c=test_data_labels.astype(np.float64), cmap='Greens', edgecolor='k', s=150, marker='^')
...
def oc_history_cols(read_path, sub_dataset, train_data, test_data, save=False):
    if "operating_condition" not in train_data.columns or "operating_condition" not in test_data.columns:
        ...
        train_data[["oc_0", "oc_1", "oc_2", "oc_3", "oc_4", "oc_5"]] = pd.DataFrame([[0, 0, 0, 0, 0, 0]],
                                                                                    index=train_data.index)
        test_data[["oc_0", "oc_1", "oc_2", "oc_3", "oc_4", "oc_5"]] = pd.DataFrame([[0, 0, 0, 0, 0, 0]],
                                                                                   index=test_data.index)

        for file in ["train", "test"]:
            if file == "train":
                groupby_traj = train_data.groupby('id', sort=False)
            else:
                groupby_traj = test_data.groupby('id', sort=False)

            additional_oc = []
            for _, data in groupby_traj:
                data = data.reset_index(drop=True)
                for i in range(1, data.shape[0]):
                    check_oc = data.at[i - 1, "operating_condition"]
                    update_cols = ["oc_" + str(int(check_oc))]
                    data.loc[i:, update_cols] = data.loc[i:, update_cols] + 1

                additional_oc.append(data)

            oc_cols = pd.concat(additional_oc, sort=False, ignore_index=True)
